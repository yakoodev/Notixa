...
Бот больше не использует slash-команды как рабочий интерфейс. Управление вынесено в inline-кнопки и пошаговые экранные сценарии.

Что осталось от текста:

- `/start` можно использовать как точку входа и сброс к главному экрану
- любой другой текст обрабатывается либо как ответ в активном сценарии, либо как инвайт-код
- ручные команды вида `/services` или `/create_service ...` больше не используются

Основные сценарии:

- `/start` открывает главный экран с разделами `📬 Мои подписки`, `🛠️ Мои сервисы` и `👑 Создатели` для супер-админа
- нажатия на inline-кнопки редактируют только то сообщение, в котором была нажата кнопка
- создание сервиса, шаблона, персонального приглашения и управление администраторами идут через кнопки и пошаговый ввод
...
3. Открой `➕ Создать сервис` и пройди пошаговый сценарий создания.
4. Скопируй `PublicId` и `ServiceKey` из ответа бота.
5. Открой `🛠️ Мои сервисы`, выбери сервис и создай общее приглашение кнопкой `🎟️ Общее приглашение`.
